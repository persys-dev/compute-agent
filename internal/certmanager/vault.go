...
// Manager handles Vault-backed certificate issuance and rotation.
type Manager struct {
	cfg    *config.Config
	logger *logrus.Entry
...
// Start ensures initial certs exist and then rotates at 80% lifetime when Vault is enabled.
...
func (m *Manager) newVaultClient() (*vault.Client, error) {
	...
	switch strings.ToLower(m.cfg.VaultAuthMethod) {
	case "token":
...
func (m *Manager) detectSANs() ([]string, []string) {
	...
	if grpcHost := strings.TrimSpace(m.cfg.GRPCAddr); grpcHost != "" && grpcHost != "0.0.0.0" {
		if ip := net.ParseIP(grpcHost); ip != nil {
			addIP(ip.String())
		} else {
			addDNS(grpcHost)
		}
...
func writeCertBundleAtomic(certPath, certPEM, keyPath, keyPEM, caPath, caPEM string) error {
	// Validate material before changing anything on disk.
